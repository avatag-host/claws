...
// is started, Claws communicates with the Panel to fetch the latest build information
// as well as get all of the details needed to parse the given Egg.
//
// This means we do not need to hit Claws each time part of the server is updated, and
// the Panel serves as the source of truth at all times. This also means if a configuration
// is accidentally wiped on Claws we can self-recover without too much hassle, so long
// as Claws is aware of what servers should exist on it.
